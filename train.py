...
EMBEDDING_DIM= config["MAX_EMBEDDING_DIM"]
...
df = pd.read_csv(input_file,sep=",,,",header=None ,names=['question','q_type'] ,engine='python')
df['q_type']=df['q_type'].str.strip()
df['question'] = df['question'].apply(lambda x: x.lower())
df['question'] = df['question'].apply((lambda x: re.sub('[^a-zA-z0-9\s?]','',x)))

NUM_CLASSES=len(df['q_type'].unique())
